...
def try_freeze(plan, split, rows, *, preprocessor):
    ...
    stages = [("dev", plan.target("dev"))] if split == "dev" else [("pilot", plan.document["pilot_episodes"]), ("diagnostic", plan.document["diagnostic_episodes"]), ("train", plan.target("train"))]
    required = []
    results = []
    by_id = {row["id"]: row for row in rows}
    for stage, count in stages:
        output = ROOT / plan.data_path(stage)
        if output.exists():
