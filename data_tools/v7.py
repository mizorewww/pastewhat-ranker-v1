...
def compact_author_fixture(raw, spec):
    ...
    if spec.get("source_constraint_version") not in {"missing-intent-required-parameter-v2", "test-missing-intent-required-parameter-v2", "dev-missing-intent-required-parameter-v2", "dev-missing-intent-unobserved-v3"} or not isinstance(raw, dict):
        return raw
