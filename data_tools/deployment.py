...
HTTP_METHOD_AUTHORING = (
    ...
    "surroundingText is visible request-editor help describing the endpoint and operation, "
    "not source code or a fill-in-the-blank quiz. The method field is empty, or its entire "
...
CODE_AUTHORING = (
    "For code-editor SELECT scenarios, provide either a real complete selectedText "
    "replacement range or only visible comment instructions followed by an empty code line. "
...
def placement_issue(episode, label=None):
    ...
    if code_editor and re.search(r"_{3,}", surrounding) and not re.search(r"_{3,}", selected):
    ...
    if code_editor and not selected and label and label["decision"] == "select":
        lines = [line.strip() for line in surrounding.splitlines() if line.strip()]
        if any(not line.startswith(("#", "//", "--")) for line in lines):
            return "Existing executable editor text has no observable replacement range; cannot certify direct paste"
    return None
