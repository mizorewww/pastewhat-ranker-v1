...
- The first formal run is registered as `ranker-v1-local-20260921`: Train 1,000, Dev 200, Calibration 400 and Test 600; the pilot uses a frozen 500-episode Train subset. [The run plan](configs/run_plan.json) is immutable and bound by SHA-256 to formal data, training, evaluation and packaging. Measured staging cost and low accepted-example yield justified this smaller initial run. The originally suggested 20,000 Train episodes have not been completed or silently relabeled as this run.
- Version 6 native-payload Kimi generation uses compact literal fixtures, two independent exhaustive candidate-verdict passes and a separate family/deployment review. Authors do not supply model-visible kinds, capabilities, imagined cursor positions or inferred intent. Native projection and student token budgeting precede teacher labeling. Formal production uses a fresh namespace; earlier staging data and the 32-row engineering set do not count toward its quotas. Invalid or semantically unsupported labels are rejected rather than edited into agreement.
- The provider's initial five-hour usage window temporarily reached its limit. Production paused, preserved its audit state and resumed only after the official console's reset and successful API responses; no alternate credential, model or identity was used. The approved teacher effort is `high`. Opt-in JSON-object requests were verified against the actual Kimi Code endpoint; response schema and semantic checks still run locally.
- Train/Dev production and the persistent GPU pipeline are moving to the registered run. The next training gate requires the complete frozen 500-episode pilot and 200-episode Dev snapshots. Main training will independently initialize seeds 42/43/44, then a new pool will supply 250 accepted hard examples mixed with 250 original samples.
- Independent Calibration/Test production uses the same registered run with separate family ownership and private examples. Test 600 preserves 50 episodes per family with 35 scheduled selectable episodes. The 95% observed recommendation-precision target, +5-point actual Top-1 improvement target and minimum family evidence requirements remain unchanged. No final Test model scores have been inspected.
