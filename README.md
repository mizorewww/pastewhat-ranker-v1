...
For this registered local run, `uv run python -m tools.install_login_recovery` installs six user LaunchAgents in `~/Library/LaunchAgents` for the next macOS login. They do not interrupt processes already running in the current session. On a later login they resume the Train/Dev and independent heldout supervisors, training, evaluation, publication and local status monitor from durable records; completed handoffs are skipped. The agents use the existing local Pi login and shared account coordinator, without embedding credentials or changing its concurrency cap. Remove those six `com.mizore.pastewhat.ranker-v1.*.plist` files after the run is complete if login recovery is no longer wanted.
